...
@app.route('/')
def index():
    photos = Photo.query.all()  # Carica tutte le foto, adattalo in base al tuo ORM
...
@app.route('/invia_usdt_da_bottone', methods=['POST'])
@login_required
def invia_usdt_da_bottone():
    indirizzo_destinatario = request.form.get('indirizzo_destinatario')
    quantita = request.form.get('quantita')
    session['indirizzo_destinatario'] = indirizzo_destinatario
    session['quantita'] = quantita
    mittente = current_user.tron_wallet_address

    private_key_hex = current_user.private_key_TRON
    private_key = PrivateKey(bytes.fromhex(private_key_hex))
    print(f"Destinatario: {indirizzo_destinatario}, Quantità: {quantita}, Chiave privata: {private_key}")

    # Reindirizza alla pagina di conferma con parametri di query nell'URL
    return redirect(url_for('conferma_invio_usdt', indirizzo_destinatario=indirizzo_destinatario, quantita=quantita))
