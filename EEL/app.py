...
# Esempio di utilizzo del client Tron
...
class Offerta(db.Model):
    ...
    def aggiorna_prezzi(self):
        # Aggiorna il campo quantita_usdt
        self.quantita_usdt = float(self.prezzo_monero) * float(self.quantita_monero)
...
def index():
    ...
    if categoria_selezionata:
        ...
        photos = Photo.query.filter_by(categoria=categoria_selezionata).order_by(Photo.id.desc()).all()
    else:
...
def load_categories_from_txt():
    path = 'categorie.txt'  # Assicurati di inserire il percorso corretto
    try:
        with open(path, 'r', encoding='utf-8') as file:
            # Legge tutto il contenuto del file, lo divide per virgole e pulisce gli spazi
            categories = file.read().strip().split(',')
            categories = [category.strip() for category in categories if category.strip()]  # Rimuove spazi extra e categorie vuote
    ...
    print('LISTA CATEGORIE:', categories)  # Stampa la lista delle categorie trovate
    return categories

# Test della funzione
load_categories_from_txt()





app.config['CATEGORIES'] = load_categories_from_txt()  # Carica e memorizza le categorie all'avvio
...
@app.route('/modifica_prezzo_offerta/<int:offerta_id>', methods=['POST'])
def modifica_prezzo_offerta(offerta_id):
    nuovo_prezzo = request.form['new_price']
    offerta = Offerta.query.get(offerta_id)
    print('offerta',offerta)
    offerta.prezzo_monero = nuovo_prezzo
    offerta.aggiorna_prezzi()  # Chiamata al metodo aggiorna_prezzi per aggiornare quantita_usdt
    return redirect(url_for('offerta_dettaglio', offerta_id=offerta_id))
...
def send_USDT_TX(mittente, destinatario, quantita, private_key):
    ...
    # Reindirizza alla pagina transazione_confermata
    return redirect(url_for('transazione_confermata', txn_id=txn.txid))
...
def conferma_invio_usdt():
    ...
    if request.method == 'POST':
        ...
        return render_template('conferma_invio_usdt.html', indirizzo_destinatario=indirizzo_destinatario,
                               quantita=quantita,private_key=private_key)
...
@app.route('/conferma_trasazione/<int:photo_id>', methods=['GET','POST'])
@login_required
...
@app.route('/conferma_trasazione_usdt/<int:photo_id>', methods=['GET', 'POST'])
@login_required
def conferma_trasazione_usdt(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    title = photo.title
    quantita = photo.price_eur
    tron_wallet_address = photo.tron_wallet_address

    if request.method == 'POST':
        session['indirizzo_destinatario'] = tron_wallet_address
        session['quantita'] = quantita
        # Reindirizza alla pagina di conferma invio USDT
        return redirect(url_for('conferma_invio_usdt'))

    return render_template('conferma_trasazione_usdt.html', title=title, quantita=quantita,
...
def messages():
    ...
    recipient_username = request.args.get('recipient')  # Otteniamo l'username del destinatario dai parametri dell'URL
